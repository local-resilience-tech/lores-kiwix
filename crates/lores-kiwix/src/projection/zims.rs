...
/// substring. The language filter matches any value in the comma-separated
/// `lang` string against the `language` column.
///
...
pub async fn list_zims_filtered(pool: &SqlitePool, criteria: FilterCriteria<'_>) -> Result<Vec<Zim>, sqlx::Error> {
    ...
    if query.is_none() && langs.is_empty() {
...
mod tests {
    ...
    fn test_data(id: &str, title: &str, description: &str, language: &str) -> ZimRegisteredDataV1 {
        ZimRegisteredDataV1 {
    ...
    async fn list_zims_filtered_matches_query_substring() {
        ...
        insert_zim(&pool, &test_data("id-1", "Golf Rules", "A book about golf", "eng"))
            .await
            .unwrap();
        insert_zim(&pool, &test_data("id-2", "Cooking", "Recipes from France", "fra"))
            .await
            .unwrap();
    ...
    async fn list_zims_filtered_matches_language() {
        ...
        insert_zim(&pool, &test_data("id-1", "Golf Rules", "A book about golf", "eng"))
            .await
            .unwrap();
        insert_zim(&pool, &test_data("id-2", "Cuisine", "Recettes de France", "fra"))
            .await
            .unwrap();
    ...
    async fn list_zims_filtered_matches_any_language_in_comma_list() {
        ...
        insert_zim(&pool, &test_data("id-2", "Cuisine", "Recettes de France", "fra"))
            .await
            .unwrap();
        insert_zim(&pool, &test_data("id-3", "Kochen", "Deutsche Rezepte", "deu"))
            .await
            .unwrap();
    ...
    async fn list_zims_filtered_combines_query_and_language() {
        ...
        insert_zim(&pool, &test_data("id-1", "Golf Rules", "A book about golf", "eng"))
            .await
            .unwrap();
        insert_zim(&pool, &test_data("id-2", "Golf en France", "A book about golf", "fra"))
            .await
            .unwrap();
    ...
    async fn list_zims_filtered_escapes_like_special_chars() {
        let pool = setup_pool().await;
        insert_zim(&pool, &test_data("id-1", "100% Golf", "A book about golf", "eng"))
            .await
            .unwrap();
        insert_zim(&pool, &test_data("id-2", "Golf Rules", "A book about golf", "eng"))
            .await
            .unwrap();
