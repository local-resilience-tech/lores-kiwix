(function () {
  ...
  function queryUrlBuilder() {
    ...
    url += Object.keys(incrementalLoadingParams)
      .map((key) => `${key}=${incrementalLoadingParams[key]}`)
      .join("&");
  ...
  function generateBookHtml(book, sort = false) {
    ...
    const tagList = tags.split(";").filter((tag) => {
    ...
    const source = getInnerHtml(book, "source") || 'local';
    const sourceClass = `source-${source}`

    divTag.innerHTML = `
            <div class="book__wrapper ${sourceClass}">
  ...
  async function getFixedMirrorbrainMagnet(magnetLink) {
    ...
    const params = new URLSearchParams(magnetLink.replaceAll("&amp;", "&").replace(/^magnet:/, ""));
  ...
  async function loadBooks() {
    ...
    return await fetch(queryUrlBuilder()).then(async (resp) => {
      ...
      const text = results ? $t("count-of-matching-books", { COUNT: results }) : "";
  ...
  async function loadAndDisplayBooksUnguarded(sort) {
    ...
    books = [...books].filter((book) => {
      return !booksToFilter.has(getInnerHtml(book, "id"));
    });
  ...
  function addTagElement(tagValue, resetFilter) {
    ...
    if (resetFilter) resetAndFilter("tag", tagValue);
  }
  ...
  function removeTagElement(resetFilter) {
    ...
    if (resetFilter) resetAndFilter("tag", "");
  }
  ...
  function updateNavVisibilityState() {
    ...
    if (Math.abs(previousScrollTop - st) <= 5 || !enableAutoHiding) return;
  ...
  async function onload() {
    ...
    document.querySelectorAll(".filter").forEach((filter) => {
      filter.addEventListener("change", () => {
        resetAndFilter(filter.name, filter.value);
      });
    });
    const tagElement = document.getElementsByClassName("tagFilterLabel")[0];
    ...
    updateVisibleParams();
    document.getElementById("kiwixSearchForm").onsubmit = (event) => {
      event.preventDefault();
    };
  ...
  window.onload = () => {
    setUserLanguage(getUserLanguage(), onload);
  };
})();
